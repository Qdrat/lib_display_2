                  ...
                  const SegmentTable *custom_table)
{
    ...
    {
        display->table = (type == LCD_TYPE_COMMON_CATHODE) ? segchar_get_default_table_cc() : segchar_get_default_table_ca();
    }
...
LcdError lcd_update(LcdDisplay *display)
{
    ...
    // Обновление ядра
    return (lcd_core_update(&display->core) == LCD_CORE_OK) ? LCD_OK : LCD_ERROR_NULL_POINTER;
}
...
LcdError lcd_clear(LcdDisplay *display)
{
    ...
    // Очищаем ядро
    return (lcd_core_clear(&display->core) == LCD_CORE_OK) ? LCD_OK : LCD_ERROR_NULL_POINTER;
}
...
LcdError lcd_set_brightness(LcdDisplay *display, uint8_t brightness)
{
    ...
    return (lcd_core_set_brightness(&display->core, brightness) == LCD_CORE_OK) ? LCD_OK : LCD_ERROR_NULL_POINTER;
}
